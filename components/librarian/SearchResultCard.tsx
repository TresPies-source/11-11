...
import { FileText, Calendar, Tag, User, ExternalLink } from "lucide-react";
...
function getSimilarityColor(similarity: number): string {
  if (similarity >= 0.9) return "text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800";
  if (similarity >= 0.8) return "text-green-600 dark:text-green-400 bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800";
  if (similarity >= 0.7) return "text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800";
...
export const SearchResultCard = memo(function SearchResultCard({
  ...
  return (
    <motion.div
      ...
      tabIndex={0}
      className={cn(
        "group bg-card rounded-lg border-2 border-border p-5 transition-all duration-200 cursor-pointer",
        "hover:border-purple-300 dark:hover:border-purple-700",
        "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-background"
      )}
    ...
    >
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-semibold text-lg text-foreground group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors line-clamp-2">
              {result.title}
            </h3>
      ...
      <div className="flex flex-wrap items-center gap-4 text-xs text-muted-foreground">
        ...
        <motion.div
          ...
          transition={{ duration: 0.2 }}
          className="ml-auto flex items-center gap-1.5 text-purple-600 dark:text-purple-400 font-medium"
        >
          <span>Open</span>
