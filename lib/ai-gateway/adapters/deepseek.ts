...
/**
 * DeepSeek provider adapter for AI Gateway.
 * Handles communication with DeepSeek API, including request formatting,
...
export class DeepSeekAdapter implements IProviderAdapter {
  ...
  /**
   * Makes an API call to DeepSeek with the given request.
   * @param request - Gateway request with messages and options
   * @param model - Model name to use (e.g., 'deepseek-chat', 'deepseek-reasoner')
  ...
  async call(request: GatewayRequest, model: string): Promise<GatewayResponse> {
    if (!this.client || !this.isConfigured) {
      throw new LLMAuthError('DeepSeek API key is invalid or not configured');
    }
    ...
    try {
      ...
      if (error?.status === 401 || error?.code === 'invalid_api_key') {
        throw new LLMAuthError('DeepSeek API key is invalid');
      }

      if (error?.status === 429 || error?.code === 'rate_limit_exceeded') {
        throw new LLMRateLimitError('DeepSeek rate limit exceeded. Please try again later.');
      }

      if (error?.status === 408 || error?.code === 'ETIMEDOUT') {
        throw new LLMTimeoutError('DeepSeek request timed out');
      }
