...
async function generateTestData() {
  ...
  for (const entry of testData) {
    ...
    console.log(`✓ Inserted ${entry.provider}/${entry.model} - ${entry.taskType} - status ${entry.statusCode}`);
  }
  ...
  console.log('\nNow navigate to http://localhost:3000/admin/ai-gateway to view the dashboard!');
}
